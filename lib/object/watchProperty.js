...
function watchSimple (bindable, property, fn) {
  var listener = bindable.on("change:" + property, fn), self;
...
function watchChain (bindable, hasComputed, chain, fn) {
  ...
  function bind (target, chain, pushValues) {
    ...
    var currentChain = [], prop, subValue, currentProperty, j, computed, hadComputed, pv, cv = target.context()
    ...
    if (hasComputed) {
      ...
      for (var i = 0, n = chain.length; i < n; i++) {
        ...
        listeners.push(target.on("change:" + (prop = currentChain.join(".")), onChange));
        target.emit("watching", prop);
      } 
